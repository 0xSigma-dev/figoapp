...
import {  updateUserDataFields } from '@/utils/indexedDB';
...
const ReferralPage = () => {
  ...
  const handleClaimPoints = async () => {
    if (pendingPoints <= 0 || !user?.id || isClaiming) return;

    setIsClaiming(true); // Start claiming process
    ...
    try {
      ...
      setIsClaiming(false); // End claiming process
    }
  };
  ...
  return (
    <div className="fixed top-0 left-0 h-full w-full bg-white dark:bg-black text-black dark:text-white p-6 z-50 flex flex-col items-center overflow-y-auto">
      <div className="flex items-center w-full shadow-2xl">
        <button
        ...
        <h2 className="text-2xl text-purple-600 font-bold ml-24">
          Invite Friends
        </h2>
      </div>
